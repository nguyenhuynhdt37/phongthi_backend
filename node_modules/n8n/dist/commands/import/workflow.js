...
function assertHasWorkflowsToImport(workflows) {
    if (!Array.isArray(workflows)) {
        throw new n8n_workflow_1.UserError('File does not seem to contain workflows. Make sure the workflows are contained in an array.');
    }
    ...
    async readWorkflows(path, separate) {
        ...
        if (separate) {
            ...
            const workflowInstances = files.map((file) => {
                ...
                const workflowInstance = di_1.Container.get(workflow_repository_1.WorkflowRepository).create(workflow);
            ...
            const workflowInstances = workflows.map((w) => di_1.Container.get(workflow_repository_1.WorkflowRepository).create(w));
