...
async function execute(index, items) {
    ...
    for (const option of config.options) {
        buttons.push((0, utils_1.createButton)(config.url, option.label, option.value, option.style));
    }
    const instanceId = this.getInstanceId();
    const bodyContent = (0, email_templates_1.createEmailBody)(config.message, buttons.join('\n'), instanceId);
    const fields = {
